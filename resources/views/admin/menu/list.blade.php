            ...
            <div class="layui-form-item">
                <div class="layui-inline tool-btn">
                    <button class="layui-btn layui-btn-small layui-btn-normal go-btn hidden-xs"
                            data-url="/admin/menu/add"><i class="layui-icon">&#xe654;</i></button>
                </div>
                ...
                @foreach($menu as $value)
                        ...
                        <td>
                            <div class="layui-inline">
                                <button class="layui-btn layui-btn-mini layui-btn-normal  go-btn"
                                        data-id="{{ $value['id'] }}" data-url="/admin/menu/edit/{{ $value['id'] }}"><i
                                            class="layui-icon">编辑</i>
                                </button>
                                <button class="layui-btn layui-btn-mini layui-btn-warm  go-btn"
                                        data-id="{{ $value['id'] }}" data-url="/admin/menu/addChild/{{ $value['id'] }}">
                                    <i
                                            class="layui-icon">添加子菜单</i>
                                </button>
                    ...
                    @foreach($value['child'] as $child)
                        <tr cate-id='{{ $child['id'] }}' fid='{{ $child['parent_id'] }}'>
                            ...
                            <td>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                @if(count($child['child']) > 0)<i class="layui-icon x-show"
                                                                  status='true'>&#xe623;</i>@else
                                    ├ @endif
                                {{ $child['name'] }}
                            </td>
                            <td>{{ $child['link'] }}</td>
                            <td><input type="text" class="layui-input x-sort" name="order" value="1"
                                       style="display:none"></td>
                            <td class="td-manage">
                                <div class="layui-inline">
                                    <button class="layui-btn layui-btn-mini layui-btn-normal  go-btn"
                                            data-id="{{ $child['id'] }}" data-url="/admin/menu/editChild/{{ $child['id'] }}">
                                        <i
                                                class="layui-icon">编辑</i>
                                    </button>
                                    <a class="layui-btn  layui-btn-mini layui-btn-danger" title="删除"
